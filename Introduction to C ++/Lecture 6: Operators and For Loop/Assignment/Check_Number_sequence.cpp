...
int main() {
    ...
    bool hasStartedIncreasing = false;
    bool isSequenceOkay =  true; // instead of using a bool simply return the from the function
    int previousNumber = sequence;
    
    while(number > 0) {		
        if(previousNumber < sequence) {
           hasStartedIncreasing = true; 
        }
        
        if(hasStartedIncreasing && previousNumber >= sequence) {
            cout << "false";
            isSequenceOkay = false; // return 0;
            break; // return 0;
        }

        number--;
	    if(number >= 1) {
            previousNumber = sequence;
            cin >> sequence;    
        }
